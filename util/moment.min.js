  ...
  formatDate(dateStr, formatter) {
    ...
    if (typeof dateStr === "string") {
      if (dateStr.indexOf(".") > -1) {
        ...
        dateStr = dateStr.slice(0, dateStr.indexOf('.')) // 删除小数点及后面的数字
      }
      //将 '-' 替换成 '/' && 删除小数点及后面的数字
      dateStr = dateStr.replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '').replace(/(-)/g, '/') // 将 '-' 替换成 '/'
    }

    return this.format(dateStr, formatter);
  },
